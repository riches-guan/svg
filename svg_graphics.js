...
module.exports = function(RED) {
    ...
    if (!fs.existsSync(jsBeautifyHtmlPath)) {
        console.log("Javascript文件 " + jsBeautifyHtmlPath + " 不存在。");
    ...
    if (!fs.existsSync(jsBeautifyCssPath)) {
        console.log("Javascript文件 " + jsBeautifyCssPath + " 不存在。");
    ...
    if (!fs.existsSync(panzoomPath)) {
        console.log("Javascript文件 " + panzoomPath + " 不存在。");
        panzoomPath = null;
    }
    ...
    if (!fs.existsSync(hammerPath)) {
        console.log("Javascript文件 " + hammerPath + " 不存在。");
        hammerPath = null;
    }
    ...
    function HTML(config) {
        ...
        if (!svgString || svgString === "") {
            svgString = String.raw`<svg width="250" height="100" xmlns="http://www.w3.org/2000/svg"> 
                            <g>
                                ...
                                <text font-weight="bold" stroke="#000000" xml:space="preserve" text-anchor="middle" font-family="Sans-serif" font-size="24" id="svg_1" y="35.85669" x="100" stroke-width="0" fill="#000000">SVG为空</text>
                            </g>
                        </svg>`;
        }
        ...
        svgString = svgString.replace(/(<text.*>)(.*)(<\/text>)/g, function(match, $1, $2, $3, offset, input_string) {
            ...
            if (!uniCode) {
                ...
                console.log("字体图标 " + iconCssClass + " 此节点不支持");
                return match;
            }
        ...
        svgString = svgString.replace(/(xlink:href="[\\\/]{1})([^"]*)(")/g, function(match, $1, $2, $3, offset, input_string) {
            if (!config.directory) {
                console.log("对于id=" + config.id + " 的svg节点，未指定映像目录。");
            ...
            if (!fs.existsSync(absoluteFilePath)) {
                console.log("指定的本地映像文件 (" + absoluteFilePath + ") 不存在。");  
                // Leave the local file path untouched, since we cannot load the specified image
                return $1 + $2 + $3; 
            }
            ...
            try {
                ...
                console.log("无法读取指定的本地映像文件 (" + absoluteFilePath + "): " + err);
                // Leave the local file path untouched, since we cannot load the specified image
                return $1 + $2 + $3;
            }
        })
...
<div class='svggraphics_` + config.nodeIdWithoutDot + `' style="width:100%; height:100%;">
   <div class='ui-svg' id='svggraphics_` + config.nodeIdWithoutDot + `' ng-init='init(` + configAsJson + `)' style="width:100%; height:100%;">` + svgString + `
                ...
                var done = ui.addWidget({
                    ...
                    beforeEmit: function(msg, value) {          
                        ...
                        if ((msg.enabled === false || msg.enabled === true) && !msg.payload) {
                            ...
                            node.error("需要消息msg.payload (msg._msgid = '" + msg._msgid + "')");
                            msg.payload = null;
                        }
                        else {
                            ...
                            if(msg.topic == "databind") {
                                ...
                                if (node.bindings.length === 0 && node.attributeBasedBindings.length === 0) {
                                    node.error("配置屏幕中或通过数据绑定文本或数据绑定值未指定绑定 (msg._msgid = '" + msg._msgid + "')");
                                    msg.payload = null;
                                }
                                else {
                                    var counter = 0;
                                    ...
                                    if (counter === 0) {
                                        node.error("此消息中没有指定的绑定字段（在配置屏幕或数据绑定文本或数据绑定值中）可用.");
                                        msg.payload = null;
                                    }
                                }
                            }
                                ...
                                if (msg.topic && (typeof payload == "string" || typeof payload == "number")) {
                                    ...
                                    if (topicParts[0] !== "update_text" || topicParts[0] !== "update_innerHTML") {
                                        node.error("仅支持msg.topic“update_text”或“update_innerHTML” (msg._msgid = '" + msg._msgid + "')");
                                        msg.payload = null;
                                    }
                                }
                                else {                                                                                          
                                    if (msg.topic) {
                                        node.warn("不支持指定的msg.topic");
                                    }
                                    ...
                                    if(Array.isArray(msg.payload)){
                                        for (var i = 0; i < msg.payload.length; i++) {
                                            ...
                                            if(typeof part === "object" && !part.command) {
                                                node.error("msg.appay数组应包含所有具有“command”属性的对象 (msg._msgid = '" + msg._msgid + "')");
                                            ...
                                            if(!node.availableCommands.includes(part.command.toLowerCase())) {
                                                node.error("msg.appay数组包含一个具有不受支持的命令属性的对象 '" + part.command + "' (msg._msgid = '" + msg._msgid + "')");
                                                msg.payload = null;
                                                break;  
                                            }
                                        }
                                        ...
                                        if(typeof msg.payload === "object") {
                                            if(!msg.payload.command) {
                                                node.error("msg.appay应包含一个具有“command”属性的对象 (msg._msgid = '" + msg._msgid + "')");
                                                msg.payload = null;
                                            }
                                            // Make sure the commands are not case sensitive anymore
                                            else if(!node.availableCommands.includes(msg.payload.command.toLowerCase())) {
                                                node.error("msg.appay包含一个具有不受支持的命令属性的对象 '" + msg.payload.command + "' (msg._msgid = '" + msg._msgid + "')");
                                                msg.payload = null;
                                            }
                                        }
                                    }
                    ...
                    initController: function($scope, events) {
                        ...
                        function handleEvent(evt, proceedWithoutTimer) {
                            ...
                            if (!svgElement) {
                                logError("找不到此SVG元素 " + evt.type + " 事件");
                                return;
                            }
                            ...
                            if (!userData) {
                                logError("没有可用的用户数据 " + evt.type + " 事件");
                                return;
                            }
                            ...
                            if (evt.type === "change") {
                                ...
                                if(msg.event.pageX !== undefined && msg.event.pageY !== undefined){
                                    ...
                                    try {
                                        ...
                                        logError("找不到此的边界客户端rect " + evt.type + " 事件");
                                        return;  
                                    }
                        ...
                        function handleJsEvent(evt, proceedWithoutTimer) {
                            ...
                            if (!svgElement) {
                                logError("找不到此  " + evt.type + " 事件的SVG元素");
                                return;
                            }
                            ...
                            if (!userData) {
                                logError("此 " + evt.type + " javascript事件没有可用的用户数据");
                                return;
                            }
                            ...
                            try {
                                ...
                                logError("javascript事件处理程序出错: " + err);
                            }
                        }
                            ...
                            if ($scope.config.clickableShapes) {
                                ...
                                $scope.config.clickableShapes.forEach(function(clickableShape) {
                                    ...
                                    if (elements.length === 0) {
                                        logError("找不到选择器 " + clickableShape.targetId + " 的元素。");
                                    }
                            ...
                            if ($scope.config.javascriptHandlers) {
                                ...
                                $scope.config.javascriptHandlers.forEach(function(javascriptHandler) {
                                    ...
                                    if (javascriptHandler.action !== "msg") {
                                        ...
                                        if (elements.length === 0) {
                                            logError("找不到选择器 '" + javascriptHandler.selector + "'的元素。");
                                        }
                        ...
                        function initializeSvg(scope) {
                            ...
                            scope.config.smilAnimations.forEach(function(smilAnimation) {
                                ...
                                if (element) {
                                    ...
                                    logError("找不到选择器 '" + smilAnimation.targetId + "'的可设置动画元素");
                                }
                            });                  
                                    ...
                                    $scope.config.javascriptHandlers.forEach(function(javascriptHandler) {
                                        ...
                                        if (javascriptHandler.action === "msg") {
                                            try {
                                                ...
                                                logError("javascript输入消息处理程序出错 (msg._msgid = '" + _msgid + "'): " + err);
                                            }
                                        }
                                    });
                                                ...
                                                attrElements.forEach(function (element) {
                                                    ...
                                                    if(attributesCSV && attrBindToCSV){
                                                        ...
                                                        if(attrNames.length != attrBindTos.length){
                                                            logError("数据绑定属性计数不同于数据绑定值计数 (msg._msgid = '" + _msgid + "')");
                                                            return;
                                                        }    
                                                        var index;
                                            ...
                                            if (topicParts.length > 1) {
                                                if (topicParts[0] == "update_text" || topicParts[0] == "update_innerHTML") {
                                                    ...
                                                    if (!elements || !elements.length) {
                                                        logError("选择器无效。找不到选择器的SVG元素 " + selector + "(msg._msgid = '" + _msgid + "')");
                                                        return;
                                                    }
                                                    elements.forEach(function (element) {
                                    ...
                                    switch (op.toLowerCase()) {
                                        case "replace_svg":
                                            if (!payload.svg || (typeof payload.svg !== "string")) {
                                                logError("有效数据无效。有效数据应该是SVG字符串 (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }
                                            ...
                                            try {
                                                ...
                                                logError("payload.svg无效。没有有效的SVG字符串 (msg._msgid = '" + _msgid + "'): " + err);
                                                return;
                                            }
                                            
                                            if (newSvg.tagName !== "svg") {
                                                logError("有效数据无效。第一个元素的标签应该是“svg” (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }
                                        ...
                                        case "add_element": // Add elements, or replace them if they already exist
                                            if (!payload.elementType) {
                                                logError("有效数据无效。未指定名为.elementType的属性 (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }
                                            ...
                                            if (parentElements.length > 1 && payload.elementId) {
                                                logError("当指定了多个父SVG元素时，不允许指定.elementId (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }
                                        ...
                                        case "remove_element":
                                            if (!payload.elementId && !payload.selector) {
                                                logError("有效数据无效。未指定名为.elementId或.selector的属性 (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }  
                                            ...
                                            elements = $scope.rootDiv.querySelectorAll(selector);
                                            if (!elements || !elements.length) {
                                                logError("选择器无效。找不到选择器的SVG元素 " + selector);
                                                return;
                                            }
                                        ...
                                        case "set_style"://same as update_style
                                            ...
                                            if (payload.style && $scope.isObject(payload.style)) {
                                                ...
                                                logError("无法更新样式！样式对象无效或未提供attributeName/attributeValue字符串 (选择器 '" + selector + "' and msg._msgid = '" + _msgid + "')");
                                                return;
                                            }
                                            break;
                                        case "update_value":  
                                            ...
                                            elements.forEach(function(element){
                                                if(element.value === undefined) {
                                                    logError("由选择器 '" + selector + "' 选择的SVG元素没有属性 (msg._msgid = '" + _msgid + "')");
                                                    return
                                                }
                                            ...
                                            elements.forEach(function(element){
                                                if (op == "update_attribute") {
                                                    if(!element.hasAttribute(payload.attributeName)) {
                                                        logError("由选择器 '" + selector + "' 选择的SVG元素没有名为 '" + payload.attributeName + "'属性 (msg._msgid = '" + _msgid + "')");
                                                        return
                                                    }
                                                }
                                        ...
                                        case "replace_all_attribute":
                                            ...
                                            if (!payload.regex) {
                                                logError("有效数据无效。应指定正则表达式 (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }   
                                            if (!payload.replaceValue) {
                                                logError("有效载荷无效。应指定替换值 (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }   
                                            selector = payload.selector || "#" + payload.elementId;
                                            elements = $scope.rootDiv.querySelectorAll(selector);
                                            ...
                                            elements.forEach(function(element){
                                                if(!element.hasAttribute(payload.attributeName)) {
                                                    logError("由 '" + selector + "' 选择的SVG元素没有名为 '" + payload.attributeName +"'的属性 (msg._msgid = '" + _msgid + "')");
                                                    return;
                                                }
                                                ...
                                                if (!regex.test(attributeValue)) {
                                                    logError("属性 " + payload.attributeName + " 的值与正则表达式不匹配 (msg._msgid = '" + _msgid + "')");
                                                    return;
                                                }
                                        ...
                                        case "trigger_animation":
                                            ...
                                            if (!payload.action) {
                                                logError("触发动画时，应该有一个.action字段 (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }
                                                    ...
                                                    switch (payload.action) {
                                                        ...
                                                        default:
                                                            try {
                                                                ...
                                                                logError("调用 " + payload.elementId + " 进出错." + payload.action + "() (msg._msgid = '" + _msgid + "')");
                                                            }
                                                            break;
                                                    }
                                                }
                                        ...
                                        case "remove_event":// remove the specified event(s) from the specified element(s)
                                            ...
                                            if (!payload.event) {
                                                logError("未指定msg.payload.event (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }

                                            if (!$scope.events.includes(payload.event)) {
                                                logError("msg.payload.event包含不受支持的事件名称 (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }
                                            ...
                                            elements.forEach(function(element) {
                                                ...
                                                if (op === "add_event") {
                                                    if (userData) {
                                                        logError("事件 " + payload.event + " 已注册 (msg._msgid = '" + _msgid + "')");
                                                    }
                                                    else {
                                                    ...
                                                    if (!userData) {
                                                        logError("事件 " + payload.event + " 尚未注册 (msg._msgid = '" + _msgid + "')");
                                                    }
                                                    else {
                                        ...
                                        case "remove_js_event":// remove the specified Javascript event(s) from the specified element(s)
                                            ...
                                            if (!$scope.events.includes(payload.event)) {
                                                logError("msg.payload.event包含不受支持的javascript事件名称 (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }
                                            ...
                                            elements.forEach(function(element) {
                                                ...
                                                if (op === "add_js_event") {
                                                    if (userData) {
                                                        logError("javascript事件 " + payload.event + " 已注册 (msg._msgid = '" + _msgid + "')");
                                                    }
                                                    else {
                                                        // Seems the event has been registered yet for this element, so let's do that now ...
                                                    ...
                                                    if (!userData) {
                                                        logError("javascript事件 " + payload.event + " 尚未注册 (msg._msgid = '" + _msgid + "')");
                                                    }
                                                    else {
                                        ...
                                        case "zoom_in":
                                            if (!$scope.panZoomModule) {
                                                logError("当设置中未启用缩放时，无法通过输入消息进行缩放 (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }
                                        ...
                                        case "zoom_by_percentage":
                                            ...
                                            if (!payload.percentage) {
                                                logError("未指定msg.payload.ppercent百分比 (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }
                                        ...
                                        case "pan_to_point":    
                                            if (!$scope.panZoomModule) {
                                                logError("当设置中未启用平移时，无法通过输入消息进行平移 (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }

                                            if (!payload.x || !payload.y) {
                                                logError("未指定任何点坐标（msg.apload.x msg.apload.y） (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }
                                        ...
                                        case "pan_to_direction": 
                                            ...
                                            if (!payload.x || !payload.y) {
                                                logError("未指定方向坐标（msg.apload.x msg.apload.y） (msg._msgid = '" + _msgid + "')");
                                                return;
                                            }
                                        ...
                                        default:
                                            if (msg.topic) {
                                                logError("不支持msg.topic '" + msg.topic + "' (msg._msgid = '" + _msgid + "')");
                                            }
                                            else {
                                                logError("不支持命令'" + payload.command + "' (msg._msgid = '" + _msgid + "')");
                                            }
                                    }
                                    
                                } 
                                catch (error) {
                                    logError("处理输入消息时出现意外错误 (msg._msgid = '" + _msgid + "'): " + error);
                                }
                            }
                            ...
                            if(topic == "custom_msg") {
                                ...
                                payload.forEach(function(val,idx){
                                    if(typeof val != "object" || !val.command) {
                                        logError("msg.epayload应包含一个具有“command”属性的对象（或一组对象） (msg._msgid = " + msg._msgid + ")");
                                    }
                                    else {   
    ...
    RED.httpAdmin.get('/ui_svg_graphics/image/:nodeid/:relativefilepath', RED.auth.needsPermission('ui_svg_graphics.read'), function(req, res) {
        ...
        if (!svgNode) {
            res.writeHead(404);
            return res.end("打不到ID为=" + nodeId + " 的SVG节点");
        }
        
        if (!svgNode.directory || svgNode.directory === "") {
            res.writeHead(404);
            return res.end("ID为 = " + nodeId + " 的节点未指定本地映像目录");
        }
    ...
    RED.httpNode.get(uiPath + "/:cmd/:value", function(req, res){
        ...
        switch (req.params.cmd) {
            ...
            case "lib":
                ...
                break
            default:
                console.log("未知命令 " + req.params.cmd);
                res.status(404).json('未知命令');
        }
    });
}
